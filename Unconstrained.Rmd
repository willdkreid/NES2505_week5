...
If we wanted to look more closely at the those species in the centre of the plot, we can look at the species scores more closely. Run the code below and look down PC1 and PC2 columns to see if you can find Trifrepe (_Trifolium_ _repens_). What are the PC1 and PC2 scores?
...
If we look at the site scores then we can see that along CA1 (x-axis) there is a big difference between the community composition at site 10 and 28. We would expect to see that the contribution for each of the species counted would be different between these two sites which represents a change in species contribution between those sites. This change in the contribution of each species to the community represents beta-diversity.

This can be illustrated more clearly by looking at what was measured to for sites 10 and 28 by looking more closely at the data.frame. First, we will give `varespec` a new column so we can easily find the two sites. Then we will extract the two sites from the data.frame so that it is easier to see the contribution of each species.
...
varespec %>% filter(site_num == "10"| site_num == "19" | site_num =="28") -> varespec2

```

You can look across these lines to see which species have different contributions.....
