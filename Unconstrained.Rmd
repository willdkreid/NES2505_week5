...
If we wanted to look at the those species in the centre of the plot, we can look at the species scores more closely. Run the code below and look down PC1 and PC2 columns to see if you can find Trifrepe (_Trifolium_ _repens_). What are the PC1 and PC2 scores?

```{r sum_pca, exercise = TRUE}
summary(dune_pca)[["species"]]
```
...
This can be illustrated more clearly by looking at what was measured for sites 10, 19 and 28 and looking more closely at these data. First, we will give `varespec` a new column so we can easily find the three sites. Then we will extract the three sites from the `data.frame` along with some key species so that it is easier to see how the contribution of each species changes from site to site.

```{r ca3, exercise=TRUE}

# we will add the row names as a column to look more closely at a couple of sites
varespec$site_num <- row.names(varespec)

# select sites 10, 19, and 28 and then four species of interest
varspec2 <- varespec %>% filter(site_num == "10"| site_num == "19" | site_num =="28") %>%  select(Cladstel, Pleuschr, Vaccmyrt, Empenigr)

# look at the species composition for the three sites
varspec2

```

If we order the different sites from highest to lowest for each species, we can see a clear pattern:
...
Site 19 is always in the middle and sites 10 and 28 are always either side of 19. If you look at your figure with the sites scores you can see where the three sites are plotted in relation to each other. The CA has reduced the dimensionality of our data from lots of *attributes* (in this case species coverage) into CA1, CA2 etc... which is now easier to see patterns in our data. The pattern of interest here being related to which sites have differing species composition. We now need to think about why there are changes in species composition. If we had environmental data then we could see whether that environmental data are related to CA1 or CA2 scores as we have done for the PCA analysis.
